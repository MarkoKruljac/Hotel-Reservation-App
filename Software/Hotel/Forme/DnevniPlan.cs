...
namespace Hotel
{
    public partial class DnevniPlan : Form
    {
        ...
        private void DohvatiDnevniPlanUseljenja()
        {
            using (var context = new PI20_021_DBEntities2())
            {
                ...
                var upit = from r in context.Rezervacija
                           ...
                           && s.ID_vrste_sobe == vS.ID_vrsta_sobe && r.Datum_pocetka.Day == DateTime.Now.Day && r.Datum_pocetka.Month == DateTime.Now.Month && r.Datum_pocetka.Year == DateTime.Now.Year
        ...
        private void DohvatiRacun()
        {
            int ID = int.Parse(dnevniPlanUseljenje_dgv.CurrentRow.Cells[0].Value.ToString());
            using (var context = new PI20_021_DBEntities2())
            {
                var upit = from ra in context.Racun
                           select ra;
            }
        }
        private void UnesiRacunUBazu()
        {
            int ID = int.Parse(dnevniPlanUseljenje_dgv.CurrentRow.Cells[0].Value.ToString());
            using (var context = new PI20_021_DBEntities2())
            {
